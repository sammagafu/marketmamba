	...
	case "STRONG_UPTREND":
		...
		if currentPrice >= ema20*0.99 && currentPrice <= ema20*1.01 && rsi < 70 {
			return &models.TradeSignal{
				...
				Type:        "BUY",
				Strength:    0.9,
				Reason:      fmt.Sprintf("STRONG_UPTREND: price near EMA20, RSI %.0f, EMA20>EMA50>EMA200", rsi),
	...
	case "STRONG_DOWNTREND":
		...
		if currentPrice >= ema20*0.99 && currentPrice <= ema20*1.01 && rsi > 30 {
			return &models.TradeSignal{
				...
				Type:        "SELL",
				Strength:    0.9,
				Reason:      fmt.Sprintf("STRONG_DOWNTREND: price near EMA20, RSI %.0f, EMA20<EMA50<EMA200", rsi),
				TriggeredAt: time.Now(),
			}
		}
	...
	case "UPTREND":
		...
		if currentPrice >= ema50*0.995 && rsi < 70 {
			return &models.TradeSignal{
				...
				Type:        "BUY",
				Strength:    0.7,
				Reason:      fmt.Sprintf("UPTREND: price above EMA50, RSI %.0f, EMA20>EMA50", rsi),
				TriggeredAt: time.Now(),
			}
		}
	...
	case "DOWNTREND":
		...
		if currentPrice <= ema50*1.005 && rsi > 30 {
			return &models.TradeSignal{
				...
				Type:        "SELL",
				Strength:    0.7,
				Reason:      fmt.Sprintf("DOWNTREND: price below EMA50, RSI %.0f, EMA20<EMA50", rsi),
				TriggeredAt: time.Now(),
			}
		}
	}
...
func SimulateScalpingOpportunity(symbol string, opportunityType string) *models.TradeSignal {
	...
	signal := sg.GenerateSignal(currentPrice, atr, ema20, ema50, ema200, rsi, bid, ask)
	if signal == nil {
		return nil
	}
	if opportunityType != "" {
		if signal.Reason != "" {
			signal.Reason = opportunityType + " — " + signal.Reason
		} else {
			signal.Reason = opportunityType
		}
	}
	return signal
}
